...
void wait_mem_done(){
    reg_file_write(MEM_OP_TYPE, READ_MEM_DONE_CODE, 0);
     while(!reg_file_read())
        reg_file_write(MEM_OP_TYPE, READ_MEM_DONE_CODE, 0);
    return;
}
void get_1k_data_from_mem(uint32_t * buff, uint16_t init_address){
    ...
    for( int i=init_address; i < (init_address + 1024); i++ ){
        reg_file_write(MEM_OP_TYPE, ADDR_MEM_CODE, i);//escribo la direccion del dato que deseo leer
        buff[counter] = reg_file_read();
        counter++; 
    }
    return;
}
